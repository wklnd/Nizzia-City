...
/*
  ...
  Usage:
    node tools/stocks/backfill.js [--days 90] [--step 15] [--symbols NIZZ,FLY]
...
const { backfillHistory } = require('../../backend/services/stockService');
...
async function main(){
  ...
  if (args.help){
    console.log('Usage: node tools/stocks/backfill.js [--days 90] [--step 15] [--symbols NIZZ,FLY]');
    process.exit(0);
  }

  await connectDB();
  try {
    const { results, since, stepMinutes } = await backfillHistory({
