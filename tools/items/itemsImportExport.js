 ...
 * Usage:
 *   # Export (pretty JSON)
 *   node tools/items/itemsImportExport.js --export items_export.json --pretty
 *
 *   # Import (upsert by custom id, do not wipe existing)
 *   node tools/items/itemsImportExport.js --import items_export.json
 *
 *   # Import after truncating the collection (DANGEROUS)
 *   node tools/items/itemsImportExport.js --import items_export.json --truncate
 */
...
(async () => {
  try {
    ...
    if (!opts.exportFile && !opts.importFile) {
      console.log('Usage:');
      console.log('  node tools/items/itemsImportExport.js --export <file> [--pretty]');
      console.log('  node tools/items/itemsImportExport.js --import <file> [--truncate] [--no-upsert]');
      process.exit(2);
    }
    await connectDB();
