...
// Import chalk safely for both v4 and v5.
let chalk = null;
try {
...
const { CRIMES, LOCATION } = require('../config/crimes/searchForCash');
...
const { PERSONALITIES, rollPersonality } = require('../config/npcPersonalities');
const {
...
const NPC_LOG_MODE = String(process.env.NPC_LOG_MODE || 'classic').toLowerCase();
const DASH_HISTORY_MAX = 12;
const dashHistory = [];
...
function colorByRatio(text, ratio) {
  const r = clamp01(ratio);
  if (r >= 0.75) return color.green(text);
  if (r >= 0.4) return color.yellow(text);
  return color.red(text);
}

function colorSparkline(values, line) {
  if (!Array.isArray(values) || values.length < 2) return color.gray(line);
  const first = Number(values[0] || 0);
  const last = Number(values[values.length - 1] || 0);
  if (last > first) return color.green(line);
  if (last < first) return color.red(line);
  return color.yellow(line);
}

function colorBar(ratio, width) {
  const r = clamp01(ratio);
  const raw = asciiBar(r, width);
  const inner = raw.slice(1, -1);
  const split = inner.indexOf('.');
  const filled = split === -1 ? inner : inner.slice(0, split);
  const empty = split === -1 ? '' : inner.slice(split);
  return `${color.gray('[')}${colorByRatio(filled, r)}${color.gray(empty)}${color.gray(']')}`;
}
...
function sparkline(values) {
  ...
  if (min === max) return chars[chars.length - 3].repeat(values.length);
  return values.map((v) => {
    const norm = (v - min) / (max - min);
    const idx = Math.max(0, Math.min(chars.length - 1, Math.round(norm * (chars.length - 1))));
    return chars[idx];
  }).join('');
}

function dashboardLine(label, value, ratio) {
  const tag = color.cyan(label.padEnd(11));
  const val = color.blueBright(String(value).padStart(8));
  return `${tag} ${val}  ${colorBar(ratio, 24)}`;
}

function printDashboard(snapshot) {
  dashHistory.push(snapshot);
  if (dashHistory.length > DASH_HISTORY_MAX) dashHistory.shift();

  const onlinePct = snapshot.npcs > 0 ? snapshot.online / snapshot.npcs : 0;
  const stockActors = snapshot.npcs > 0 ? snapshot.stockActors / snapshot.npcs : 0;
  const crimeActors = snapshot.npcs > 0 ? snapshot.crimeActors / snapshot.npcs : 0;
  const jobsActors = snapshot.npcs > 0 ? snapshot.jobsActors / snapshot.npcs : 0;
  const drugsActors = snapshot.npcs > 0 ? snapshot.drugsActors / snapshot.npcs : 0;

  const volumeTrend = sparkline(dashHistory.map(d => d.volume));
  const crimeTrend = sparkline(dashHistory.map(d => d.crimeMoney));
  const actionTrend = sparkline(dashHistory.map(d => d.totalActions));

  console.log(`${color.gray(ts())} ${color.magenta('[npc-dashboard]')} ${color.blueBright('tick')}`);
  console.log(`${color.cyan('mode'.padEnd(11))} ${color.blueBright(NPC_LOG_MODE)}`);
  console.log(dashboardLine('online', `${snapshot.online}/${snapshot.npcs}`, onlinePct));
  console.log(dashboardLine('stocks', `${snapshot.buys}/${snapshot.sells}`, stockActors));
  console.log(dashboardLine('crime', `${snapshot.crimeAttempts}`, crimeActors));
  console.log(dashboardLine('jobs', `${snapshot.worked}`, jobsActors));
  console.log(dashboardLine('drugs', `${snapshot.drugsSold}g`, drugsActors));
  console.log(`${color.cyan('volume'.padEnd(11))} ${color.magenta(`$${fmtMoney(snapshot.volume)}`)}  ${color.gray('trend')} ${colorSparkline(dashHistory.map(d => d.volume), volumeTrend)}`);
  console.log(`${color.cyan('crime$'.padEnd(11))} ${color.magenta(`$${fmtMoney(snapshot.crimeMoney)}`)}  ${color.gray('trend')} ${colorSparkline(dashHistory.map(d => d.crimeMoney), crimeTrend)}`);
  console.log(`${color.cyan('actions'.padEnd(11))} ${color.blueBright(String(snapshot.totalActions))}  ${color.gray('trend')} ${colorSparkline(dashHistory.map(d => d.totalActions), actionTrend)}`);
}

const ACTIVITY_KEYS = [
  'stocks', 'gym', 'bank', 'crime', 'work', 'drugs',
  'education', 'casino', 'realEstate', 'business',
  'pets', 'market', 'items',
];

function hashToUnit(x) {
  const s = String(x ?? '0');
  let h = 2166136261;
...
function getProfile(player) {
  const key = `${player.id}:${player.user}`;
  const u0 = hashToUnit(`${key}:0`);
  const u1 = hashToUnit(`${key}:1`);
  const u2 = hashToUnit(`${key}:2`);

  const timezoneOffset = Math.floor(u0 * 25) - 12; // -12..+12
  const chronotype = u1 < 0.33 ? 'early' : (u1 < 0.66 ? 'normal' : 'night');
  const sessionBurstiness = 0.75 + u2 * 0.6; // 0.75..1.35

  return { timezoneOffset, chronotype, sessionBurstiness };
}

function getLocalHour(now, timezoneOffset) {
  const utcHour = now.getUTCHours();
  return (utcHour + timezoneOffset + 24) % 24;
}

function isOnlineNow(player, now) {
  const profile = getProfile(player);
  const hour = getLocalHour(now, profile.timezoneOffset);
  const minute = now.getUTCMinutes();
  const tickNoise = hashToUnit(`${player.id}:${now.toISOString().slice(0, 16)}`);

  let awake = false;
  if (profile.chronotype === 'early') awake = inWindow(hour, 6, 22);
  else if (profile.chronotype === 'night') awake = inWindow(hour, 10, 3);
  else awake = inWindow(hour, 8, 24);

  // Slight activity waves across the hour so NPCs do not all act at once.
  const wave = Math.sin(((minute / 60) * Math.PI * 2) + (tickNoise * Math.PI));
  const base = awake ? 0.58 : 0.03;
  const chance = clamp(base + wave * 0.08, 0.01, 0.9);
  return Math.random() < chance;
}

function activityContextMultiplier(player, activity) {
  const money = Number(player.money || 0);
  const energy = Number(player.energyStats?.energy || 0);
  const nerve = Number(player.nerveStats?.nerve || 0);

  if (activity === 'gym') return energy >= 20 ? 1 : 0.15;
  if (activity === 'crime') return nerve >= Number(CRIMES.search_for_cash?.nerveCost || 2) ? 1 : 0.1;
  if (activity === 'stocks') return money > 500 ? 1 : 0.1;
  if (activity === 'bank') return money > 1000 ? 1 : 0.2;
  if (activity === 'realEstate' || activity === 'business') return money > 50000 ? 1 : 0.2;
  if (activity === 'casino') return money > 5000 ? 1 : 0.25;
  return 1;
}

function planNpcActions(npcs, now) {
  const sets = Object.fromEntries(ACTIVITY_KEYS.map(k => [k, new Set()]));
  let onlineCount = 0;

  for (const p of npcs) {
    if (!isOnlineNow(p, now)) continue;
    onlineCount++;

    const personality = PERSONALITIES[p.npcPersonality] || PERSONALITIES.allrounder;
    const profile = getProfile(p);
    const scored = [];

    for (const key of ACTIVITY_KEYS) {
      const base = Number(personality.weights?.[key] || 0.2);
      const ctx = activityContextMultiplier(p, key);
      const jitter = 0.8 + Math.random() * 0.5;
      const score = base * ctx * jitter * profile.sessionBurstiness;
      if (score > 0.08) scored.push({ key, score });
    }

    if (!scored.length) continue;
    scored.sort((a, b) => b.score - a.score);

    // Most players do 1-3 focused things per session tick.
    const maxActions = 1 + (Math.random() < 0.5 ? 1 : 0) + (Math.random() < 0.2 ? 1 : 0);
    const picks = scored.slice(0, maxActions);
    for (const pick of picks) sets[pick.key].add(p);
  }

  return { sets, onlineCount };
}
...
async function tickNpcActions(){
  ...
  const { sets, onlineCount } = planNpcActions(npcs, now);
  const stocksSet = Array.from(sets.stocks);
  const gymSet = Array.from(sets.gym);
  const bankSet = Array.from(sets.bank);
  const crimeSet = Array.from(sets.crime);
  const workSet = Array.from(sets.work);
  const drugSet = Array.from(sets.drugs);
  const eduSet = Array.from(sets.education);
  const casinoSet = Array.from(sets.casino);
  const estateSet = Array.from(sets.realEstate);
  const bizSet = Array.from(sets.business);
  const petSet = Array.from(sets.pets);
  const marketSet = Array.from(sets.market);
  const itemSet = Array.from(sets.items);

  console.log(`${ts()} [npc] online this tick: ${onlineCount}/${npcs.length}`);
  ...
  // Summary logs (classic or dashboard mode)
  const vol = buyValue + sellValue;
  const priced = Object.values(prices).filter(v => Number(v) > 0).length;

  if (NPC_LOG_MODE === 'dashboard') {
    const totalActions =
      buys + sells + crimeAttempts + worked + drugsPlanted + eduEnrolled +
      eduCompleted + casinoSpins + estBought + estUpgraded + bizBought +
      bizUpgraded + petsBought + itemsUsed + mktListed + mktBought;

    printDashboard({
      npcs: npcs.length,
      online: onlineCount,
      stockActors: stocksSet.length,
      crimeActors: crimeSet.length,
      jobsActors: workSet.length,
      drugsActors: drugSet.length,
      buys,
      sells,
      crimeAttempts,
      crimeMoney,
...
function scheduleNpcActions(){
  ...
  setTimeout(() => {
    const warmUpSummary = [ `${ts()}`, color.green('[npc]'), color.blueBright('[warm-up]') ];
