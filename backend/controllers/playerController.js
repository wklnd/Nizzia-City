...
function normalizeProfileImages(input) {
...
function withProfileImageFallback(obj) {
  const links = Array.isArray(obj?.profileImages) && obj.profileImages.length
    ? obj.profileImages
    : [DEFAULT_PROFILE_IMAGE];
  return {
    ...obj,
    profileImages: links,
    profileImage: links[0],
  };
}
...
async function createPlayer(req, res) {
  try {
    ...
    const { name, gender, profileImages } = req.body;
    if (!name || !gender) {
      return res.status(400).json({ error: 'name and gender are required' });
    }

    const normalizedImages = normalizeProfileImages(profileImages);
    if (Array.isArray(profileImages) && (normalizedImages.length < 1 || normalizedImages.length > 10)) {
      return res.status(400).json({ error: 'profileImages must contain 1-10 valid image links' });
    }
    ...
    const player = new Player({ user: userId, name, id: nextId, gender, profileImages: normalizedImages });
    await player.save();
    return res.status(201).json(withProfileImageFallback(player.toObject()));
  } catch (err) {
...
async function getPlayerByUser(req, res) {
  try {
    ...
    return res.json(withProfileImageFallback(player.toObject()));
  } catch (err) {
...
// PATCH /api/player/me/profile-images
// Body: { images: string[] } OR { profileImages: string[] }
async function updateMyProfileImages(req, res) {
  try {
    const userId = req.authUserId;
    const raw = Array.isArray(req.body?.images) ? req.body.images : req.body?.profileImages;
    if (!Array.isArray(raw)) {
      return res.status(400).json({ error: 'images must be an array of image links' });
    }

    const links = normalizeProfileImages(raw);
    if (links.length > 10) {
      return res.status(400).json({ error: 'images must contain 0-10 valid image links' });
    }

    const player = await Player.findOneAndUpdate(
      { user: userId },
      { $set: { profileImages: links } },
      { new: true }
    );
    if (!player) return res.status(404).json({ error: 'Player not found' });
    ...
    console.error('Update profile images error:', err);
    return res.status(500).json({ error: 'Failed to update profile images' });
  }
}

module.exports = { createPlayer, getPlayerByUser, updateMyProfileImages };
...
async function getPublicProfile(req, res) {
  try {
    ...
    const profile = {
      ...
      profileImages: Array.isArray(p.profileImages) && p.profileImages.length ? p.profileImages : [DEFAULT_PROFILE_IMAGE],
      profileImage: (Array.isArray(p.profileImages) && p.profileImages[0]) || DEFAULT_PROFILE_IMAGE,
      vitals: {
        health: p.health,
